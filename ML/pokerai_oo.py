    ...
    def __init__(self, feature_count, class_count, save_file, name):
        ...
        with self.graph.as_default():
            with tf.variable_scope(name):
                ...
                self.hidden_1_count = round((feature_count + class_count) * 0.66)
                self.hidden_2_count = self.hidden_1_count

                self.input_layer = tf.placeholder(tf.float32, [None, feature_count], 'input')

                self.weights_1 = tf.get_variable('weights_1', shape=[feature_count, self.hidden_1_count], initializer=tf.contrib.layers.xavier_initializer()) # tf.Variable(tf.zeros([FEATURE_COUNT, HIDDEN_COUNT]))
                ...
                self.weights_2 = tf.get_variable('weights_2', shape=[self.hidden_1_count, self.hidden_2_count], initializer=tf.contrib.layers.xavier_initializer()) # tf.Variable(tf.zeros([HIDDEN_COUNT, CLASS_COUNT]))
                self.biases_2 = tf.Variable(tf.constant(0.1, shape=[self.hidden_2_count]))
                ...
                self.output_layer = tf.matmul(self.hidden_layer_4, self.weights_5) + self.biases_5
